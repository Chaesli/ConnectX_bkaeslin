...
def my_agent(observation, configuration):
    ...
    def winning_next(observation, player_num, cfg_rows, cfg_cols, cfg_inarow):
        ...
        for i in range(cfg_rows):
            for j in range(cfg_cols - cfg_inarow - 1):
                if observation.board[j + cfg_cols * i] == player_num \
                    ...
                    if j != (cfg_cols-1)*(i+1) and observation.board[j + cfg_cols * i + 3] == 0:
                        return j + 3
        ...
        for i in range(cfg_cols):
            for j in range(cfg_rows - cfg_inarow - 1):
                if observation.board[i + cfg_rows * j] == player_num \
                        and observation.board[i + cfg_rows * (j + 1)] == player_num \
                        and observation.board[i + cfg_rows * (j + 2)] == player_num \
                        and observation.board[i + cfg_rows * (j + 3)] == 0:
                    return i
        else:
            return -99
